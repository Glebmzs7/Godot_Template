...
import git_ops
import notifier
from version import Version, VALID_STATUSES
...
class RepoWatcher(FileSystemEventHandler):
    ...
    def _commit_and_push(self, reason: str) -> None:
        try:
            ...
            # TODO: реальный расчёт push_count/task_label — здесь заглушка (0,0), обвязку версии
            # нужно достроить, когда определитесь с тем, как task_label выставляется вручную
            # разработчиком (см. чат: "пользователь сам обозначает что меняется").
            version = Version(
            ...
            notifier.notify(f"AutoSync [{self.name}]", f"Синхронизировано: {reason}")
        except git_ops.GitError as e:
